...
const replaceSymbols = (text: string | undefined, showBg: boolean, bgColor: string): React.ReactNode => {
  if (!text) return null;
          ...
          nodes.push(
            <span 
              ...
              style={{ 
                ...
                padding: showBg ? '0 0.25rem' : '0',
                margin: showBg ? '0 0.125rem' : '0',
                filter: 'saturate(1.2)' // Same filter for all symbols
              }}
            >
              {config.fontChar}
            </span>
    ...
    // If it's already a React element, we need to process its children
      ...
      const newChildren = children.map(child => {
        if (typeof child === 'string' && child.includes(config.symbol)) {
          return replaceSymbols(child, showBg, bgColor);
        }
        return child;
      });
    ...
    // Handle array of nodes (from previous replacements)
      ...
      currentNode = currentNode.map((node, index) => {
        if (typeof node === 'string' && node.includes(config.symbol)) {
          return replaceSymbols(node, showBg, bgColor);
        }
        return node;
      });
    }
...
const UnitStatCard = ({ unit, showSymbolBg = true, symbolBgColor = "rgba(40, 40, 40, 0.7)" }: UnitStatCardProps) => {
  // Use replaceSymbols with the background settings
  const processText = (text: string | undefined) => replaceSymbols(text, showSymbolBg, symbolBgColor);
          ...
          <div className="grid grid-cols-5 text-center gap-2">
            {Object.entries(unit.stats).map(([key, value]) => (
              <div key={key} className="bg-black/60 p-2 rounded-md">
                ...
                <p className="text-warcrow-text">{processText(String(value))}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <div key={profileIndex} className="bg-black/60 p-3 rounded-md my-2">
              ...
              {profile.ranged && (
                <div className="border-l-4 border-amber-700 p-2 my-2">
                  ...
                  {profile.ranged.modifier && (
                    <p className="text-warcrow-text"><strong className="text-warcrow-gold">Modifier:</strong> {processText(profile.ranged.modifier)}</p>
                  )}
                    ...
                    <div className="mt-1">
                      ...
                      {profile.ranged.switches.map((sw, i) => (
                        <p key={i} className="text-warcrow-text text-sm ml-4">
                          <span className="text-warcrow-gold">{processText(sw.value)}:</span> {processText(sw.effect)}
                        </p>
                      ))}
                    </div>
                  )}
              ...
              <div className="border-l-4 border-red-700 p-2 my-2">
                ...
                {profile.attack.modifier && (
                  <p className="text-warcrow-text"><strong className="text-warcrow-gold">Modifier:</strong> {processText(profile.attack.modifier)}</p>
                )}
              ...
              <div className="border-l-4 border-blue-700 p-2 my-2">
                ...
                {profile.defense.modifier && (
                  <p className="text-warcrow-text"><strong className="text-warcrow-gold">Modifier:</strong> {processText(profile.defense.modifier)}</p>
                )}
              ...
              <div className="mt-2">
                ...
                {unit.abilities.skill.map((ability, i) => (
                  <div key={i} className="ml-4 mt-1">
                    {ability.name && <strong className="text-warcrow-gold">{ability.name}:</strong>} <span className="text-warcrow-text">{processText(ability.description)}</span>
                  </div>
                ))}
              </div>
            )}
