    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (!chaptersData || chaptersData.length === 0) {
          console.warn("No chapters data returned from the database");
        ...
        if (!sectionsData) {
          console.warn("No sections data returned from the database");
          // Return chapters with empty sections
        ...
        console.error("Error in useRules hook:", error);
