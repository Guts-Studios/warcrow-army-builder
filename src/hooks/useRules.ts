...
import { translations } from "@/i18n/translations"; // Add this import
...
// Spanish translation for the Characters chapter is already implemented

// Add any other Spanish chapter translations as needed
...
export const useRules = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Add the Characters chapter translation (already implemented)
      const charactersChapter = language === 'en' ? {
        ...
        sections: [
          ...
          // ... keep existing code (character chapter sections)
          {
      ...
      translatedChapters.push(charactersChapter);
      ...
      const typedChapters: Chapter[] = chaptersData.map(chapter => {
        ...
        if (translations[titleKey]) {
          title = translations[titleKey][language];
        }
